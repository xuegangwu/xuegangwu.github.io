...
app.get('/api/components/hierarchical', (req, res) => {
    ...
    sql += ' ORDER BY type, brand, model';
    const topLevel = db.prepare(sql).all(...params);
    // Get all sub-components
    const allChildren = db.prepare('SELECT * FROM components WHERE parentId IS NOT NULL ORDER BY type, brand, model').all();
...
app.get('/api/components/:id', (req, res) => {
    ...
    // Get children
    const children = db.prepare('SELECT * FROM components WHERE parentId = ?').all(req.params.id);
    res.json({
        success: true,
        data: {
            ...row,
...
app.get('/api/designs', (req, res) => {
    const rows = db.prepare('SELECT * FROM designs ORDER BY updatedAt DESC').all();
...
app.put('/api/designs/:id', (req, res) => {
    ...
    WHERE id=?
  `).run(name, description || '', JSON.stringify(requirements), JSON.stringify(components || []), JSON.stringify(bom || []), totalCost || 0, aiSuggestion || '', status || 'draft', now, req.params.id);
    const row = db.prepare('SELECT * FROM designs WHERE id = ?').get(req.params.id);
    res.json({
        success: true,
        data: {
...
app.post('/api/ai/suggest', async (req, res) => {
    ...
    res.json({
        ...
        data: {
            recommendations,
            bom,
            totalCost: bom.reduce((sum, item) => sum + item.totalPrice, 0),
        },
    });
});
...
app.get('/api/health', (req, res) => {
    const components = db.prepare('SELECT COUNT(*) as c FROM components').get();
    const designs = db.prepare('SELECT COUNT(*) as c FROM designs').get();
    res.json({
        success: true,
        data: {
            status: 'online',
...
// ── Helper Functions ─────────────────────────────────────────────────────────
function suggestComponents(req) {
    ...
    const battery = db.prepare('SELECT * FROM components WHERE type = ? ORDER BY price ASC LIMIT 1').get('battery');
    if (battery) {
        suggestions.push({
    ...
    const pcs = db.prepare('SELECT * FROM components WHERE type = ? ORDER BY price ASC LIMIT 1').get('pcs');
    if (pcs) {
        suggestions.push({
            componentId: pcs.id,
            type: 'pcs',
    ...
    // BMS
    const bms = db.prepare('SELECT * FROM components WHERE type = ? ORDER BY price ASC LIMIT 1').get('bms');
    if (bms) {
        suggestions.push({
            componentId: bms.id,
            type: 'bms',
    ...
    // EMS
    const ems = db.prepare('SELECT * FROM components WHERE type = ? ORDER BY price ASC LIMIT 1').get('ems');
    if (ems) {
        suggestions.push({
            componentId: ems.id,
            type: 'ems',
    ...
    // Fire protection
    const fire = db.prepare('SELECT * FROM components WHERE type = ? ORDER BY price ASC LIMIT 1').get('fire');
    if (fire) {
        suggestions.push({
            componentId: fire.id,
            type: 'fire',
    ...
    const cabinet = db.prepare('SELECT * FROM components WHERE type = ? ORDER BY price ASC LIMIT 1').get('cabinet');
    if (cabinet) {
        suggestions.push({
...
function calculateBOM(suggestions) {
...
async function generateAIResponse(message, designId) {
